...
# Script JS amélioré pour garantir la détection des sorties
st.components.v1.html("""
    ...
    // Fonction pour déclencher le bouton Streamlit caché
    function triggerCheat() {
        // On cherche tous les boutons
        const buttons = window.parent.document.querySelectorAll('button');
        for (const btn of buttons) {
            // On cherche celui qui contient notre marqueur
    ...
    // 2. Détection perte de focus (Clic hors fenêtre ou Alt+Tab)
...
st.markdown("""
    ...
    /* On utilise position absolute pour le sortir de l'écran tout en le gardant dans le DOM pour le JS */
...
# Ce bouton est invisible mais essentiel.
# Chaque clic (déclenché par JS) incrémente le compteur.
if st.button("INTEGRITY_TRIGGER", key="cheat_trigger"):
    st.session_state.cheats += 1
    # Pas de st.rerun() ici pour éviter le clignotement de l'interface
...
def get_algeria_time_str(timestamp):
    ...
    # Timestamp est généralement en secondes UTC
...
# --- 6. GÉNÉRATION DE RAPPORTS (OFFICIEL COMPLET) ---
class ReportPDF(FPDF):
    def header(self):
        ...
        # Heure locale Algérie pour l'impression
...
def generate_final_report_pdf(stats, results_df):
    ...
    pdf.cell(90, 10, f"Candidats Presents (Copies rendues): {stats['present']}", 1)
    ...
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 10, "2. LISTE DETAILLEE (Par ordre d'arrivée)", 0, 1)
    ...
    # results_df est déjà trié par timestamp
    for index, row in results_df.iterrows():
        ...
        # Alerte Triche en rouge
        obs = "RAS"
        try:
            alertes = int(row['Alertes'])
            if alertes > 0:
                obs = f"ALERTE ({alertes})"
                pdf.set_text_color(255, 0, 0) # Rouge vif
                pdf.set_font("Arial", "B", 10)
        except: pass
        
        pdf.cell(50, 10, obs, 1, 1, 'C')
        
        # Reset style standard pour la prochaine ligne
...
def teacher_dash():
    ...
    # TRI DES RÉSULTATS PAR TIMESTAMP (ARRIVÉE)
    # Les premiers arrivés (timestamp plus petit) en premier dans la liste ?
    # Ou les derniers arrivés en haut ? Généralement on veut voir les derniers arrivés en haut.
    # Demande utilisateur: "affichage par ordre temporaire d'arrivée".
    # Interprétation: Chronologique (le premier qui finit est le premier sur la liste).
    ...
    with tab2:
        ...
        # Total participants (ayant rendu)
        col_m[1].metric("Copies Rendues", len(r_list))
    ...
    # --- ONGLET 3 : GESTION (AVEC ANTI-DOUBLON STRICT) ---
    with tab3:
        c1, c2 = st.columns(2)
        with c1:
                ...
                try:
                    ...
                    for name in df.iloc[:, 0].dropna():
                        ...
                        if norm_name not in existing_names: 
                            ...
                            count_added += 1
                            # Ajout au set local pour éviter doublons DANS le fichier excel lui-même
                            existing_names.add(norm_name)
                    
                    fetch_dashboard_data.clear() # Vider le cache pour rafraîchir la liste
                    
                    if count_added > 0:
                        st.success(f"{count_added} nouveaux étudiants ajoutés avec succès.")
                    else:
    ...
    with tab4:
        if r_list:
            # Préparation des données affichables
            data_for_df = []
            for r in r_list:
                ...
                data_for_df.append({
                    ...
                    # Conversion Timestamp -> Heure Algérie
            ...
            # 3. TRI STRICT PAR ORDRE D'ARRIVÉE (croissant)
            ...
            # Génération du PDF avec la liste TRIÉE
            ...
            st.download_button(
                ...
                help="Contient : Stats, Liste triée par heure, Notes et Alertes triche"
            )
            
            st.markdown(f"### Liste des {len(r_list)} copies (Ordre d'arrivée)")
            
            # Affichage interactif
            sel = st.dataframe(
                df_res.drop(columns=["ID", "timestamp"]), 
                use_container_width=True, 
                on_select="rerun", 
                selection_mode="single-row"
            )
            
            # Affichage Détaillé d'une copie
            if sel and sel.selection.rows:
                selected_row_idx = sel.selection.rows[0]
                # Récupération via iloc car df_res est trié
                ...
                if full_data:
                    ...
                    st.markdown(f"**Remis à :** {get_algeria_time_str(full_data.get('timestamp',0))} (Heure Algérie)")
        ...
        with st.form("exam_sub"):
            ...
            if st.form_submit_button("TERMINER"):
                ...
                get_col('results').add({
                    ...
                    # IMPORTANT: On enregistre le timestamp serveur actuel
                    "timestamp": time.time()
                })
